...
const control = (paramName: string | null, paramId: number | null, widget: LayoutControl['widget'] = 'knob', rawWidget?: string): LayoutControl => ({ label: paramName ?? 'Graph', paramName, paramId, widget, rawWidget });
...
describe('deriveMegaTapGraphs', () => {
  it('binds the Tap Control graph to its page-local tap pattern controls', () => {
    const layout: DeviceLayout = { family: 'MEGATAP', pages: [{ name: 'Tap Control', rows: [{ controls: [
