<script lang="ts">
  ...
  import type { BlockSummary, CabState, CabSlot, EnumParam, LayoutControl } from '$lib/api/types';
...
{#if embedded || (editor.editorOpen && sel && cat)}
  ...
  >
    ...
    <div class="card" class:sheet={editor.isMobile && !embedded}>
      ...
      <!-- header: close only. Search lives in a full-width bar above the canvas; the block's identity
...
<style>
  ...
  /* Full-width search bar above the canvas: spans the whole stage so the field gets real width, and it
     reads directly above the page tabs / canvas it filters. */
  .sbar {
    flex: none;
    display: flex;
    align-items: center;
