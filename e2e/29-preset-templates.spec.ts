...
/**
 * "New preset" — the bare + in the top-bar preset cluster opens a menu: start from a template, save
 * the current buffer as a template, or clear the buffer to a blank preset ("start from zero"). The
 * widget must sit to the right of the scene/pencil widget and before Save. Both template dialogs are
 * registry-owned modals (open on click, close on Escape); no ForgeFX → their list/body shows the
 * empty/error state, which is fine — this spec covers the trigger, the menu, and the modal shells.
 */
test.describe('New preset', () => {
  test('the + sits right of the scene widget and its menu opens the template picker', async ({ page }) => {
    ...
    await expect(menu.getByRole('menuitem', { name: 'Save preset as template…' })).toBeVisible();
  ...
  test('the menu opens the save-as-template dialog', async ({ page }) => {
    await bootCleanWorkbench(page);

    await page.locator('[data-widget="axis.widget.newPreset"] button').click();
    const menu = page.locator('.aw-context-menu[role="menu"]');
    await menu.getByRole('menuitem', { name: 'Save preset as template…' }).click();

    const dialog = page.getByRole('dialog', { name: 'Save preset as template' });
    await expect(dialog).toBeVisible();
    // No ForgeFX / no detected unit → no templates folder, so the dialog degrades to guidance.
    await expect(dialog.getByText('No templates folder configured')).toBeVisible();
