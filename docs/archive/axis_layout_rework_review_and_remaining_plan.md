...
| T35 | P2 | **Axis ↔ workbench feature-home audit + migration** (AFTER full implementation, operator decision 2026-07-06). Rule during implementation: **keep every production feature — the design missing a feature (e.g. preset/block search with recents, favorites, all) never justifies removing it.** Afterwards, audit which capabilities are app-generic and belong in the standalone workbench framework (search with filtering/favorites/recents, query bars, pickers, list virtualization, toasts, context menus) vs. Axis-specific (device semantics), and migrate the generic ones into `src/lib/workbench/` with data/adapters supplied by Axis — same seam pattern as the estW sizing provider. | `src/lib/workbench/**`, `src/lib/axis-workbench/**`, `library.svelte.ts`, `PresetBrowser.svelte` | audit doc with per-feature home decision; generic features importable without Axis | audit review + boundary tests | 0.7 |
